...
const ProjectCard: React.FC<ProjectCardProps> = ({ initiativeType,project, onSave, mode }) => {
...
console.log("Response",response.ID.toString())      
  ...
  const validateFields = () => {
    ...
    if (!newProject.Status) {
      errors.Status = 'Title is required';
      isValid = false;
      newErrors.Status = 'Status Year is required.';
    }
    ...
    if (newProject.EndYear && isNaN(Number(newProject.EndYear))) {
      ...
      <Box sx={{ display: 'flex', gap: 2, marginBottom: 3 }}>
          <Box ml={1}>
              <TextField
                ...
                size="small"
              >
                {approvedOptions.map((approved) => (
                  <MenuItem key={approved.key} value={approved.value}>
                    {approved.value}
                  </MenuItem>
                ))}
              </TextField>
